...
fn main() {
    ...
    if let Err(err) = build_slides(out) {
...
fn build_slides(out_dir: &Path) -> io::Result<()> {
    ...
    fn to_rust(&self) -> String {
        match self {
            ...
            SlideDef::Image {
                ...
                "Slide::Image(ImageSlide {{ title: {}, image: {}, text: {} }})",
                rust_string(title),
                rust_string(image),
                markdown_to_rust(markdown)
            ),
...
fn parse_slide(lines: &[&str]) -> Result<SlideDef, String> {
    ...
    if let Some((_, image)) = parse_image_syntax(first_line.trim())? {
