...
fn build_assets(out_dir: &Path, slides: Vec<SlideDef>) -> io::Result<()> {
    ...
    writeln!(out_file, "use crate::indexed_image::IndexedImage;")?;
    ...
    for png_path in entries {
        ...
        quantize_image(&png_path, width, height, out_dir, stem)?;

        writeln!(
            out_file,
            "static {const_name}_PALETTE: [u16; 256] = include!(concat!(env!(\"OUT_DIR\"), \"/{stem}.pal.rs\"));"
        )?;
        writeln!(
            out_file,
            "pub static {const_name}: IndexedImage = IndexedImage::new(\
                include_bytes!(concat!(env!(\"OUT_DIR\"), \"/{stem}.idx\")), \
                &{const_name}_PALETTE, {width});"
        )?;
    ...
    writeln!(
        out_file,
        "pub fn resolve_image(name: &str) -> Option<&'static IndexedImage> {{"
    )?;
...
fn quantize_image(png_path: &Path, width: u32, height: u32, out_dir: &Path, stem: &str) -> io::Result<()> {
    ...
    for i in 0..256 {
        let [r, g, b, _] = nq.lookup(i).expect("palette index out of range");
        palette[i] = ((r as u16 >> 3) << 11) | ((g as u16 >> 2) << 5) | (b as u16 >> 3);
    }

    // Map each pixel to nearest palette index
    let indices: Vec<u8> = pixels.chunks(4)
        .map(|px| nq.index_of(px) as u8)
        .collect();

    // Write index data
    let idx_path = out_dir.join(format!("{stem}.idx"));
    fs::write(&idx_path, &indices)?;

    // Write palette as Rust array literal
    let pal_path = out_dir.join(format!("{stem}.pal.rs"));
    let mut f = File::create(&pal_path)?;
    write!(f, "[")?;
    for (i, &color) in palette.iter().enumerate() {
        if i > 0 { write!(f, ",")?; }
        write!(f, "0x{color:04x}")?;
    }
    write!(f, "]")?;
